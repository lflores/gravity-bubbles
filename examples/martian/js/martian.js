...
function forceChannelNames(node){
	// 8 bbubles
...
function updateViewports(){
	...
	var onDragonLoadedHandler = function(loadedFragment){
		...
		TweenMax.set("#dragonDropship", {y:-15, opacity: 0});
		...
		setTimeout( function(){
			...
			var flash = function(){
				...
				var fadeOuCallback = function(){
					setTimeout( function(){
						smoke.animate({"opacity":0}, 500, mina.easeout);
						thrusters.animate({"opacity":0}, 500, mina.easeout);
					}, 4000);
				}
				
				smoke.animate({"opacity":opacity}, 500, mina.easeout);
				thrusters.animate({"opacity":opacity}, 500, mina.easeout);
			...
			var onComplete = function(){
				TweenMax.to("#dragonDropship", 1, {y:-5, rotation: +2, delay: 250});
				animateFire(dragonGroup);
			}

			TweenMax.to("#dragonDropship", 1, {x:-10, y:+35, rotation: -10, opacity: 1, onComplete: onComplete}); 
	...
	var onSettlementLoadedHandler = function(loadedFragment){
		...
		var flipRover = function(){
			...
			rover.animate({transform: roverBackTransform}, 65000, atHomeCallback);
		}
		...
		var resetRover = function(){
			...
			rover.animate({transform: roverTransform}, 65000, atDestinationCallback);
		}
...
// DATA
  ...
  var animateFire = function(world){
    ...
    var particle = {x: 0, y: 0, skin: null};
    ...
    if ( particles.length === 0 ) {
	    ...
	    _.times(10, function(index){
	      ...
	      if ( index < 4 ) {
	      	skin = dragon.rect(dragonBox.x,fireBase,particleWidth,particleHeight).attr({fill:"orange", filter: blurFilter}); //.transform("r190");
	      } else if ( index >= 4 && index < 8) {
	      	skin = dragon.rect(dragonBox.x + 20,fireBase,particleWidth,particleHeight).attr({fill:"darkred", filter: blurFilter});
	      } else {
	      	skin = dragon.rect(dragonBox.x + 40,fireBase,particleWidth,particleHeight).attr({fill:"orange", filter: blurFilter});//.transform("r310");
	      }
    	...
    	_.each( particles, function(particle, index){
    		...
    		if ( index < 4 ) {
	      		particle.skin.attr({x: dragonBox.x, y: fireBase, opacity: 1, fill:"orange", filter: blurFilter});
	      	} else if ( index >= 4 && index < 8) {
	      		particle.skin.attr({x: dragonBox.x + 20, y: fireBase, opacity: 1, fill:"red", filter: blurFilter});
	      	} else {
	      		particle.skin.attr({x: dragonBox.x + 40, y: fireBase, opacity: 1, fill:"orange", filter: blurFilter});
	      	}
    	});
    ...
    _.each( particles, function(particle, index){
    	...
    	if ( index < 4 ) {
      		top = {x:dragonBox.x - spread, y: 100, opacity: 0, fill: "#000"};
  		} else if ( index >= 4 && index < 8) {
  			top = {y: 80, opacity: 0, fill: "#000"};
  		} else {
  			top = {x:dragonBox.x + spread, y: 100, opacity: 0, fill: "#000"};
 	 	}
